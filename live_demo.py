...
def main():
    ...
    ap.add_argument("--qty", type=int, default=None, help="覆寫每筆股數")
    ...
    while time.time() < t_end and trades < max_trades:
        ...
        try:
            trade = data.get_stock_latest_trade(
                StockLatestTradeRequest(symbol_or_symbols=sym, feed=DataFeed.IEX))[sym]
        ...
        if len(prices) < slow_n:
            print("  [{:%H:%M:%S}] {}  {:.2f}  │ 暖機 {}/{}".format(
                now, sym, price, len(prices), slow_n))
            time.sleep(poll)
            continue
        ...
        if cross_up:
            ...
            mark = "  (糾纏中)"
        print("  [{:%H:%M:%S}] {}  {:.2f}  │ 快線 {:.3f}  慢線 {:.3f}  差 {:+.3f}  │ {}  {}{}".format(
            now, sym, price, fast, slow, diff, state_zh, wait_zh, mark))
        ...
        if fire:
            ...
            print("   | 送出市價{}單  {}  {} 股".format(side_zh, sym, qty))
            try:
            ...
            fill_zh = "{:.2f} 美元".format(fill) if fill else "尚未回報({:.2f} 參考價)".format(price)
            ...
            if side == OrderSide.BUY:
                ...
                fields = [
                    ("標的", sym, True),
                    ("動作", "買進 {} 股".format(qty), True),
                    ("觸發原因", reason, False),
                    ("成交均價", "{:.2f} 美元".format(ref), True),
                ...
                print("   | 進場均價  {:.2f}   出場均價  {:.2f}".format(entry_price or 0, ref))
                print("   | 本筆{}  {:+,.2f} 美元 ({:+.2f}%)".format(tag, trade_pl, trade_pct))
                print("   | 累計損益  {:+,.2f} 美元".format(realized_pl))
                fields = [
                    ("標的", sym, True),
                    ("動作", "賣出 {} 股".format(qty), True),
                    ("觸發原因", reason, False),
                    ("進場均價", "{:.2f} 美元".format(entry_price or 0), True),
                    ("出場均價", "{:.2f} 美元".format(ref), True),
    ...
    if state == "long":
        print("  目前狀態:持有 {} 股(尚未平倉,損益未計入)".format(qty))
    else:
        print("  目前狀態:空手")
    try:
